...
function createMainBundler (mainFiles, transforms, events, options) {
  let bundler = newify(mainFiles, transforms, events, options);
...
function createMinifiedBundler (mainFiles, transforms, events, options) {
  ...
  let bundler = newify(minifiedFiles, transforms, events, options);
...
function createTestBundler (mainFiles, transforms, events, options) {
  ...
  let bundler = newify(testFiles, transforms, events, options);
...
function newify (fileSet, transforms, events, options) {
  ...
  // Some additional options may be set,
  // depending on which Browserify transforms are being used
  Object.assign(browserifyOptions, transforms.getBrowserifyOptions(browserifyOptions));
