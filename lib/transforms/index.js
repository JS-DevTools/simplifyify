...
Transforms.prototype.apply = function applyTransforms (bundler, options) {
  try {
    ...
    // Add the banner last
    // NOTE: `bannerify()` returns an updated config, which saves us
    // from re-reading the banner.txt file multiple times
    this.bannerify = applyBannerify(bundler, this.manifest, this.bannerify);
  }
  catch (e) {
