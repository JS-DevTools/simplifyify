...
describe('simplifyify --test', function() {
  ...
  it('should add code-coverage to multiple files', function(done) {
    cli.run('es5/lib/**/index.js --test --outfile es5/dist/',
      function(err, stdout) {
        ...
        expect(stdout).to.contain('es5/lib/say/index.js --> es5/dist/say/index.test.js');
        ...
        assert.fileContents('es5/dist', ['index.test.js', 'say/index.test.js'], function(contents) {
          assert.hasPreamble(contents);
          assert.isMinified(contents, true);
          assert.noSourceMap(contents);
          assert.hasCoverage(contents);
  ...
  it('should append ".test" when renaming output files', function(done) {
    cli.run('es5/lib/**/*.js --test --minify --debug --outfile es5/dist/*.foo.es5',
      function(err, stdout) {
        if (err) {
          return done(err);
        }
