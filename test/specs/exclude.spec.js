...
describe('simplifyify --exclude', () => {
  it('should not exclude anything if nothing matches', (done) => {
    cli.run('es5/lib/**/*.js --exclude es5/lib/**/*-foo.js --outfile es5/dist/', (err, stdout) => {
      if (err) {
        return done(err);
      }
  ...
  it('should exclude a single file', (done) => {
    cli.run('es5/lib/**/*.js --exclude es5/lib/say/index.js --outfile es5/dist/', (err, stdout) => {
      if (err) {
        return done(err);
      }
  ...
  it('should exclude multiple files', (done) => {
    cli.run('es5/lib/**/*.js --exclude es5/lib/**/index.js --outfile es5/dist/', (err, stdout) => {
      if (err) {
        return done(err);
      }
