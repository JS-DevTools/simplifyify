...
const addTransforms = require('./add-transforms');
...
function writeBundles (mainFiles, events, options) {
  ...
  // Read the project manifest (package.json file) by traversing up from the entry file path
  ...
  if (options.bundle) {
    bundlers.push(createMainBundler(mainFiles, manifest, events, options));
  }

  if (options.minify) {
    bundlers.push(createMinifiedBundler(mainFiles, manifest, events, options));
  }

  if (options.test) {
    bundlers.push(createTestBundler(mainFiles, manifest, events, options));
  }
 ...
 * @returns {Browserify}
 */
function createMainBundler (mainFiles, manifest, events, options) {
  let bundler = newify(mainFiles, events, options);
  addTransforms(bundler, manifest);
  return bundler;
}
 ...
 * @returns {Browserify}
 */
function createMinifiedBundler (mainFiles, manifest, events, options) {
  ...
  addTransforms(bundler, manifest, { minify: true });
  return bundler;
}
 ...
 * @returns {Browserify}
 */
function createTestBundler (mainFiles, manifest, events, options) {
  ...
  addTransforms(bundler, manifest, { minify: true, test: true });
  return bundler;
}
